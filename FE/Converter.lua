...
88. ~8~ 88b  d88   .88.        j88.         88booo. 88b  d88 88   88 
 Y888P  ~Y8888P' Y888888P      888888D      Y88888P ~Y8888P' YP   YP  CONVERTER
	...
	repeat task.wait() until script:FindFirstChild("DeltaKeyboard")
	...
	local CAS = {Actions={},BindAction=function(self,name,fun,touch,...)
		self.Actions[name] = fun and {Name=name,Function=fun,Keys={...}} or nil
	end}
	...
	Event.Parent = NLS([[
	script.Parent.DeltaKeyboard.Enabled = true
	local Event = script:WaitForChild("UserInput_Event")
local Mouse = game.Players.LocalPlayer:GetMouse()
local UIS = game:GetService("UserInputService")
local CAS = game:GetService("ContextActionService")
local input = function(io,RobloxHandled,override)
	if RobloxHandled then return end
	local uit = io.UserInputType
	if override then
		uit = Enum.UserInputType.MouseButton1
	end
	--Since InputObject is a client-side instance, we create and pass table instead
	Event:FireServer({KeyCode=io.KeyCode,UserInputType=uit,UserInputState=io.UserInputState,Hit=Mouse.Hit,Target=Mouse.Target})
end
UIS.InputBegan:Connect(input)
UIS.InputEnded:Connect(input)

_G.keypress = function(keycode)
	input({KeyCode = keycode, UserInputType = Enum.UserInputType.Touch, UserInputState = Enum.UserInputState.Begin},false,false)
end

_G.keyrelease = function(keycode)
	input({KeyCode = keycode, UserInputType = Enum.UserInputType.Touch, UserInputState = Enum.UserInputState.End},false,false)
end

if not UIS.TouchEnabled then
	script.Parent.DeltaKeyboard:Destroy()
	warn("Destroyed keyboard bc the client is pc/console.")
end
...
local function CreateFloatingKey(keybind, text)
	...
	key.MouseButton1Up:Connect(function()
		_G.keyrelease(Enum.KeyCode[keybind])
	end)
	key.MouseButton1Down:Connect(function()
		_G.keypress(Enum.KeyCode[keybind])		
	end)
		...
		if string.find(keys.Name, "-") then
			...
			keys.MouseButton1Up:Connect(function()
				...
				if IsSelecting.Value == true then
					...
					_G.keyrelease(Enum.KeyCode[key])
				end
			end)
			...
			keys.MouseButton1Up:Connect(function()
				...
				elseif ToggleShift.Value == true then
					if IsSelecting.Value == false then
						_G.keypress(Enum.KeyCode[keys.Name])
		...
		elseif keys.Name == "LeftShift" then
			...
			keys.MouseButton1Up:Connect(function()
				if IsSelecting.Value == true then
					...
					_G.keyrelease(Enum.KeyCode[keys.Name])
				end
			end)
		end		
	end
...
script.Parent.ToggleShift.MouseButton1Up:Connect(function()
	...
	-- StarterGui.DeltaKeyboard.CreateKey.CreateKey
	local createKey = NLS([===[
...
create.MouseButton1Up:Connect(function()
	if textbox.Text ~= "" then
		...
		key.MouseButton1Up:Connect(function()
			_G.keyrelease(Enum.KeyCode[textbox.Text])
		end)
		key.MouseButton1Down:Connect(function()
			_G.keypress(Enum.KeyCode[textbox.Text])
		end)
		key.Close.MouseButton1Up:Connect(function()
			key:Destroy()
		end)
	...
	-- StarterGui.DeltaKeyboard.FloatingIcon.FloatIcon
	local floaticon = NLS([===[

...
local function UpdatePlayer()
	local PlayerAncestor = script:FindFirstAncestorWhichIsA("Player")
	if PlayerAncestor then
		Plr=PlayerAncestor
		return PlayerAncestor
	else
		for _,v in game:GetService("Players"):GetPlayers() do
			if v.Character and script:IsDescendantOf(v.Character) then
				Plr=v
				return v
			end
		end
...
script.AncestryChanged:Connect(UpdatePlayer)
UpdatePlayer()
NLS([==[
...
h.ToolTip = "A very old shotgun script. The name is SB Shotgun."
